function plotSurfaceTrack_withHarps(glider, gpsSurfT, path_shp, latlim, lonlim, ...
    harps_shp, targetsFile)
...
h(5) = scatterm([harps_shp.Lat], [harps_shp.Lon], 60, 'Marker', 's', ...
    'MarkerFaceColor', [0 0 0], 'MarkerEdgeColor', [0 0 0], ...
    'DisplayName', 'HARP');
textm(harps_shp(1).Lat-0.06, harps_shp(1).Lon-0.04, 'E', 'FontSize', 14)
textm(harps_shp(2).Lat-0.05, harps_shp(2).Lon-0.08, 'U', 'FontSize', 14)
textm(harps_shp(3).Lat+0.05, harps_shp(3).Lon-0.08, 'N', 'FontSize', 14)
textm(harps_shp(4).Lat+0.05, harps_shp(4).Lon+0.02, 'H', 'FontSize', 14)
