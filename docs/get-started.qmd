...
### Download __*agate*__ from [GitHub](https://github.com/sfregosi/agate) 
  ...
  - **Option 2:** Clone the repository using GitHub or GitHub Desktop 
    - This is the easiest way stay up to date with the latest improvements to this actively maintained package. However, it requires that you regularly check for updates from GitHub and comes with risks as it may be buggy
  ...
  - **Option 3:** Fork the repository if you'd like to modify the tools and contribute back to **agate** — see the [Contribute](contribute.qmd) page for the full fork-and-clone-and-pull-request workflow.
...
```default
├── sgXXX_XXX_LocYEAR (mission folder)
  ├── piloting (optional subdir)
    ├── flightStatus
    ├── basestationFiles
    └── profiles
```
...
mkdir(fullfile(CONFIG.path.mission, 'profiles')); % where to store post-processing
```
...
% define your basestation and flightStatus paths
path_bsLocal = fullfile(CONFIG.path.mission, 'basestationFiles');
path_status = fullfile(CONFIG.path.mission, 'flightStatus');
