...
Convert the recorded `.dat` files to `.flac` or `.wav` to preview/browse in your preferred analysis software. 

```{matlab}

# initialize agate with an empty configruation file
CONFIG = agate('agate_config_empty.cnf');
# will prompt to select - use agate_config_empty.cnf example in agate/settings or
...
# convertWispr(CONFIG, 'inDir', 'E:/wisprFiles', 'outDir', 'E:/wav', 'outExt', '.wav')
```