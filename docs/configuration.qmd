...
The top section is [required](#required-configuration-settings) to initialize __*agate*__ and use the most basic functions. The remaining sections are optional depending on what __*agate*__ functionality is desired, including interfacing with the [basestation](#basestation-configuration-file), working with acoustic data outputs, and plotting maps. Save this file with a unique name for each glider and mission. Descriptions of each configuration item are included in the example file as comments. 
...
CONFIG.sgVer = XX.XX; % 66.12, 66.14, 67.00 or 67.01, firmware version displayed on glider start up
