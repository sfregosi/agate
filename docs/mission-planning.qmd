...
A bathymetric basemap is optional and is toggled on or off with the `bathy` argument. Bathymetry will be plotted if `bathy` is set to 1 or to a fullfile path to a bathymetry `.tiff` file. The specified file must cover the area of the track. If set to 1 it will first look for a file specified in the configuration file (`CONFIG.map.bathyFile`) otherwise it will prompt to select a file. See [Dependencies - Basemap rasters](index.html#basemap-rasters) for more info on how to select and download bathymetry rasters. 
...
See [Dependencies - Basemap rasters](index.html#basemap-rasters) for more info on how to select and download bathymetry rasters. 
