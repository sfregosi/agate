...
NOAA's [Uncrewed Systems Research Transition Office](https://www.omao.noaa.gov/uncrewed-systems/about-noaa-uncrewed-systems-operations) funded additional development into a more broadly applicable and user-friendly tool box that could be used by all interested in conducting glider-based passive acoustic surveys for marine mammals. 
...
This code was either developed or most recently updated and tested with MATLAB version 2024b, but has undergone some testing with 2020b.  

#### MATLAB toolboxes

- Signal Processing Toolbox
- Mapping Toolbox  
Currently __*agate*__ requires the MATLAB Mapping toolbox. In the future, we may try modify the mapping tools to work with the freely available `m_map` package to make it more accessible, but are not there yet!
...
Some of the acoustic processing functionality relies on a few open-access MATLAB tools. For convenience these have also been included within __*agate*__. They include the following folders within the `utils` folder: 

- `eos-wispr3` contains a modified version of the `read_wispr_file` function created by the WISPR developer, Embedded Ocean Systems, and available on GitHub:  [wispr3](https://github.com/embeddedocean/wispr3). Modifications can be tracked on a fork of this repository: [sfregosi-noaa/wispr3 - sf branch](https://github.com/sfregosi-noaa/wispr3/tree/sf).
- `martin_et_al_2021` contains functions for calculating hybrid millidecade frequency bands. These functions were copied from the supplementary materials from Martin et al. (2021) Erratum: Hybrid millidecade spectra: A practical format for exchange of long-term ambient sound data, *JASA Express Letters* 1, 081201 doi: [10.1121/10.0005818](https://doi.org/10.1121/10.0005818).
