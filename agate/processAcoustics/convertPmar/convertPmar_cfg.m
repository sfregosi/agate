...
% inDir = { 'E:\SoCal2020\SG607card1raw\' ...    % cell array example
  ...
  for dj = 1 : length(pmarDirs)                 % ...and process each one
    ...
    for fi = 1 : length(pmarFiles)
      ...
      if (~pmarFiles(fi).isdir && length(nm) >= 6 && contains(nm(3:6), dig4) ...
        ...
        [sams,nChan,~,sRate,nLeft,dt,hdr] = pmarIn(inFile, 0, inf, []);
        ...
        if (decim > 1 && isempty(lpFilt))
          lpFilt = designfilt('lowpassfir', ...   % a low-pass FIR filter
            'SampleRate',               sRate, ...
            'PassbandFrequency',        sRate/2 / decim * relativeCutoffFreq, ...
            'StopbandFrequency',        sRate/2 / decim, ...
        ...
        for dk = 1 : length(outDir)
          ...
          audiowrite(outFile, sams / 32768, round(sRate));
