function [seq,nLeft] = readBinary(filename, offset, n, typ, fmt)
% READBINARY    Open a file, read data from it, close it, and return the data.
% 
% seq = readBinary(filename, offset, n, typ)
%    From the file filename, skip offset bytes from the beginning, then
%    read n objects of type typ and return as a column vector.  If filename
%    is actually a number (not a string), use it as the file number to read 
%    from, and don't mess with opening/closing it.  typ is one of the types
%    listed in fread, with these exceptions:
%
...
%
% seq = readBinary(filename, offset, n, typ, machineformat)
...
elseif (strncmp(typ, 'float*', 6)),   len = str2num(typ(7:length(typ)));
elseif (strncmp(typ, 'real*', 5)),    len = str2num(typ(6:length(typ)));
else
