function [ltsa, ltsaP, ltsaParams] = ...
    gliderLTSA_calcLTSAsingleHour(ltsaParams,soundFiles,sysSensFile)
%
...
% updated 2019 11 04 S. Fregosi to deal with 2 gain values

% 1 hour scale
binSec = 3600/ltsaParams.tAvg;
...
ltsaParams.pTimeDN      = [ltsaParams.hrLims(1):ltsaParams.tAvg/86400:ltsaParams.hrLims(2)];
ltsaParams.pTimeSec     = [0:ltsaParams.tAvg:binSec-1]';
...
for f = ltsaParams.hrFiles
    ...
    for a = aRange
        ...
        if dAvg == 0 % divides evenly into file size
            ...
            if a == 1 && ltsaParams.fNumAvg(f) == 1 % only one sample for this file
                nSamp = ltsaParams.fNumSamp(f);
                ltsaParams.avgDur(s) = nSamp/ltsaParams.fs;
                yi = 1;
            elseif a == ltsaParams.fNumAvg(f)% its the last average so not full sampPerAvg
                % so resample the avgslice before PLUS this partial bit.
                nSamp = ltsaParams.fNumSamp(f) - ...
        ...
        % --------------------------------------------------------------
        %         data = data*vref;
        %         data = data/(10^(12/20)); % gain adjustment
        %         data = data/(10^(-164.5/20)); % hydrophone adjustment
...
%             elseif size(frqSysSens,2) == 3 % two gain settings
%                 gainChangeDate = sysSens.gainChangeDate;
%                 if currDN < gainChangeDate
%                     ssC = 2;
%                 elseif currDN > gainChangeDate
%                     ssC = 3;
%                 end
%                 sysSensI = interp1(frqSysSens(:,1),frqSysSens(:,ssC),ltsaParams.fBins,'pchip');
%                 %                 bandAvgdB = 10*log10(bandAvg);
%                 bandAvgdB = bandAvg;
%                 bandAvgdB = bandAvgdB - sysSensI; % dB re 1 uPa^2/Hz
%             end
        ...
        if a == ltsaParams.fNumAvg(f) && f < ltsaParams.hrFiles(end)-1
            ...
            if gap > ltsaParams.tAvg
                p = p + round(gap); % this works with 1 sec resolution...not otherwise?
                fprintf(1,'gap! ');
            elseif gap < -1
                p = p - 1;
                fprintf(1,'step back! ');
...
% % check fig
% figure(40)
% imagesc(ltsaParams.timeDN,ltsaParams.fBins,ltsa)
% set(gca,'YDir','normal');
% caxis([20 120]);
% title([datestr(ltsaParams.dayNum) ' Hr ' num2str(ltsaParams.hrNum)]);
% 
% figure(41)
% imagesc(ltsaParams.pTimeSec, ltsaParams.fBins, ltsaP, 'AlphaData', ~isnan(ltsaP));
% set(gca,'YDir','normal');
% caxis([20 120]);
% title([datestr(ltsaParams.dayNum) ' Hr ' num2str(ltsaParams.hrNum)]);
