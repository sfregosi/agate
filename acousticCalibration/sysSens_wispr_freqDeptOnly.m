function frqSysSens = sysSens_wispr_freqDeptOnly(instrDeplStr,savePath)
...
if length(gain) == 1
    sysSens = paGain + hydFilt + antiAli + 20*log10(1/vref);
...
elseif length(gain) > 1
    ...
    for g = 1:length(gain)
        sysSens(g,:) = paGain + hydFilt + antiAli + 20*log10(1/vref);
    end
    frqSysSens = [frqSys' sysSens'];
...
if ~isempty(savePath)
    save([savePath instrDeplStr '_sysSens_freqDeptOnly.mat'],'frqSysSens');
    if exist('gainChangeDate')
        save([savePath instrDeplStr '_sysSens_freqDeptOnly.mat'],'frqSysSens','gainChangeDate');
    end
